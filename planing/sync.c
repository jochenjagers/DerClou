    ...
    if (direction)
    {
        ...
        switch (gowhere)
        {
            case DIRECTION_LEFT:
                ...
                if ((animate & PLANING_ANIMATE_FOCUS) && (id == CurrentPerson)) lsScrollLandScape((ubyte)-1);
...
void plSync(ubyte animate, uint32_t targetTime, uint32_t times, ubyte direction)
{
    ...
    for (seconds = 0; seconds < times; seconds++)
    {
        for (i = 0; i < PersonsNr; i++)
        {
            ...
            if (CurrentTimer(plSys) == targetTime) continue;

            if (!direction && (i < BurglarsNr))
            {
                if (GetMaxTimer(plSys) < (targetTime + times - seconds)) continue;
            }
            ...
            else
            {
                ...
                if (!action)
                {
                    ...
                    if (animate & PLANING_ANIMATE_STD) livAnimate(Planing_Name[i], ANM_STAND, 0, 0);
                }
                else
                {
                    ...
                    {
                        case ACTION_GO:
                            xpos = plXMoveSync(i, xpos, animate, direction,
                                               ActionData(action, struct ActionGo *)->Direction);
                            ypos = plYMoveSync(i, ypos, animate, direction,
                                               ActionData(action, struct ActionGo *)->Direction);
                            break;
                            ...
                            if (ActionStarted(plSys))
                            {
                                if (plIsStair(ActionData(action, struct ActionUse *)->ItemId))
                                {
                                    if (direction)
                                        livLivesInArea(
                                            Planing_Name[i],
                                            StairConnectsGet(ActionData(action, struct ActionUse *)->ItemId,
                                                             ActionData(action, struct ActionUse *)->ItemId));
                                    else
                                ...
                                {
                                    if (direction)
                                        lsSetObjectState(ActionData(action, struct ActionUse *)->ItemId,
                            ...
                            if (ActionEnded(plSys))
                            {
                                ...
                                else
                                {
                                    if (direction)
                                    {
                                        ...
                                        {
                                            if (!CHECK_STATE(
                                                    lsGetObjectState(ActionData(action, struct ActionUse *)->ItemId),
                                                    Const_tcLOCK_UNLOCK_BIT))
                                            {
                                                lsSetObjectState(ActionData(action, struct ActionUse *)->ItemId,
                                                                 Const_tcLOCK_UNLOCK_BIT, 1);

                                                if (((Tool)dbGetObject(ActionData(action, struct ActionUse *)->ToolId))
                                                        ->Effect &
                                                    Const_tcTOOL_OPENS)
                                                {
                                                    lsSetObjectState(ActionData(action, struct ActionUse *)->ItemId,
                                        ...
                                        if (!plIgnoreLock(ActionData(action, struct ActionUse *)->ItemId))
                                        {
                                                    ...
                                                    Const_tcLOCK_UNLOCK_BIT))
                                            {
                                                if ((((LSObject)dbGetObject(
                                                          ActionData(action, struct ActionUse *)->ItemId))
                                                         ->Type == Item_Fenster) &&
                                                    !ActionData(action, struct ActionUse *)->ToolId)
                                                {
                                                    lsWalkThroughWindow(
                                                        (LSObject)dbGetObject(
                                                            ActionData(action, struct ActionUse *)->ItemId),
                                    ...
                                    if (plIgnoreLock(ActionData(action, struct ActionUse *)->ItemId))
                                    {
                                        ...
                                        if (CHECK_STATE(state, Const_tcON_OFF))
                                        {
                                            ...
                                            if (plIgnoreLock(ActionData(action, struct ActionUse *)->ItemId) ==
                                                PLANING_POWER)
                                            {
                                                lsSetSpotStatus(ActionData(action, struct ActionUse *)->ItemId,
                                                                LS_SPOT_ON);
                        ...
                        case ACTION_TAKE:
                            ...
                            {
                                if (direction)
                                    lsSetObjectState(ActionData(action, struct ActionTake *)->ItemId,
                                                     Const_tcIN_PROGRESS_BIT, 1);
                                else
                                    lsSetObjectState(ActionData(action, struct ActionTake *)->ItemId,
                                                     Const_tcIN_PROGRESS_BIT, 0);
                            ...
                            if (ActionEnded(plSys))
                            {
                                ...
                                {
                                    weightLoot =
                                        ((Loot)dbGetObject(ActionData(action, struct ActionTake *)->LootId))->Weight;
                                    volumeLoot =
                                        ((Loot)dbGetObject(ActionData(action, struct ActionTake *)->LootId))->Volume;

                                    lsSetObjectState(ActionData(action, struct ActionTake *)->ItemId,
                                                     Const_tcIN_PROGRESS_BIT, 0);

                                    if ((ActionData(action, struct ActionTake *)->ItemId >= 9701) &&
                                        (ActionData(action, struct ActionTake *)->ItemId <= 9708))
                                    {
                                    ...
                                    {
                                        if (CHECK_STATE(
                                                lsGetObjectState(ActionData(action, struct ActionTake *)->ItemId),
                                                Const_tcTAKE_BIT))
                                        {
                                            lsTurnObject(
                                                (LSObject)dbGetObject(ActionData(action, struct ActionTake *)->ItemId),
                                    ...
                                    {
                                        newValue = GetP(dbGetObject(ActionData(action, struct ActionTake *)->ItemId),
                                                        hasLoot(i),
                                                        dbGetObject(ActionData(action, struct ActionTake *)->LootId));

                                        if (Ask(dbGetObject(OL_NR(GetNthNode(BurglarsList, i))), take_RelId,
                                                dbGetObject(ActionData(action, struct ActionTake *)->LootId)))
                                        {
                                            oldValue =
                                                GetP(dbGetObject(OL_NR(GetNthNode(BurglarsList, i))), take_RelId,
                                                     dbGetObject(ActionData(action, struct ActionTake *)->LootId));

                                            SetP(dbGetObject(OL_NR(GetNthNode(BurglarsList, i))), take_RelId,
                                                 dbGetObject(ActionData(action, struct ActionTake *)->LootId),
                                                 oldValue + newValue);
                                        }
                                        else
                                    ...
                                    UnSet(dbGetObject(ActionData(action, struct ActionTake *)->ItemId), hasLoot(i),
                                          dbGetObject(ActionData(action, struct ActionTake *)->LootId));
                                ...
                                else
                                {
                                        ...
                                        (ActionData(action, struct ActionTake *)->ItemId <= 9708))
                                    {
                                        lsAddLootBag(xpos, ypos,
                                                     ActionData(action, struct ActionTake *)->ItemId - 9700);
                                        Planing_Loot[ActionData(action, struct ActionTake *)->ItemId - 9701] = 1;
                                        SetP(dbGetObject(ActionData(action, struct ActionTake *)->ItemId), hasLoot(i),
                                             dbGetObject(ActionData(action, struct ActionTake *)->LootId),
                                    ...
                                    else
                                    {
                                        ...
                                        SetP(dbGetObject(ActionData(action, struct ActionTake *)->ItemId), hasLoot(i),
                                             dbGetObject(ActionData(action, struct ActionTake *)->LootId),
                                             GetP(dbGetObject(ActionData(action, struct ActionTake *)->ItemId),
                                                  hasLoot_Clone_RelId,
                                                  dbGetObject(ActionData(action, struct ActionTake *)->LootId)));
                                    }
                        ...
                        case ACTION_DROP:
                            ...
                            {
                                if (direction)
                                    lsSetObjectState(ActionData(action, struct ActionDrop *)->ItemId,
                                                     Const_tcIN_PROGRESS_BIT, 1);
                                else
                                    lsSetObjectState(ActionData(action, struct ActionDrop *)->ItemId,
                                                     Const_tcIN_PROGRESS_BIT, 0);
                            ...
                            if (ActionEnded(plSys))
                            {
                                ...
                                {
                                    weightLoot =
                                        ((Loot)dbGetObject(ActionData(action, struct ActionDrop *)->LootId))->Weight;
                                    volumeLoot =
                                        ((Loot)dbGetObject(ActionData(action, struct ActionDrop *)->LootId))->Volume;

                                    lsSetObjectState(ActionData(action, struct ActionDrop *)->ItemId,
                                                     Const_tcIN_PROGRESS_BIT, 1);

                                    if ((ActionData(action, struct ActionDrop *)->ItemId >= 9701) &&
                                        (ActionData(action, struct ActionDrop *)->ItemId <= 9708))
                                    {
                                        lsAddLootBag(xpos, ypos,
                                                     ActionData(action, struct ActionDrop *)->ItemId - 9700);
                                        Planing_Loot[ActionData(action, struct ActionDrop *)->ItemId - 9701] = 1;
                                        SetP(dbGetObject(ActionData(action, struct ActionDrop *)->ItemId), hasLoot(i),
                                             dbGetObject(ActionData(action, struct ActionDrop *)->LootId),
                                             GetP(dbGetObject(OL_NR(GetNthNode(PersonsList, i))), take_RelId,
                                                  dbGetObject(ActionData(action, struct ActionDrop *)->LootId)));
                                    }
                                    else
                                    {
                                        ...
                                        {
                                            lsTurnObject(
                                                (LSObject)dbGetObject(ActionData(action, struct ActionDrop *)->ItemId),
                                                LS_OBJECT_VISIBLE, LS_COLLISION);
                                            lsSetObjectState(ActionData(action, struct ActionDrop *)->ItemId,
                                                             Const_tcACCESS_BIT, 1);
                                        }
                                        ...
                                        SetP(dbGetObject(ActionData(action, struct ActionDrop *)->ItemId), hasLoot(i),
                                             dbGetObject(ActionData(action, struct ActionDrop *)->LootId),
                                             GetP(dbGetObject(ActionData(action, struct ActionDrop *)->ItemId),
                                                  hasLoot_Clone_RelId,
                                                  dbGetObject(ActionData(action, struct ActionDrop *)->LootId)));
                                    }
                                ...
                                else
                                {
                                    ...
                                    {
                                        newValue = GetP(dbGetObject(ActionData(action, struct ActionDrop *)->ItemId),
                                                        hasLoot(i),
                                                        dbGetObject(ActionData(action, struct ActionDrop *)->LootId));
                                    ...
                                    UnSet(dbGetObject(ActionData(action, struct ActionDrop *)->ItemId), hasLoot(i),
                                          dbGetObject(ActionData(action, struct ActionDrop *)->LootId));
                        ...
                        case ACTION_OPEN:
                            if (ActionStarted(plSys))
                            {
                                ...
                                {
                                    if (direction)
                                        lsSetObjectState(ActionData(action, struct ActionOpen *)->ItemId,
                                                         Const_tcIN_PROGRESS_BIT, 1);
                                    else
                                        lsSetObjectState(ActionData(action, struct ActionOpen *)->ItemId,
                                                         Const_tcIN_PROGRESS_BIT, 0);
                            ...
                            if (ActionEnded(plSys))
                            {
                                if (direction)
                                {
                                    ...
                                    plCorrectOpened(
                                        (LSObject)dbGetObject(ActionData(action, struct ActionOpen *)->ItemId), 1);
                                }
                                else
                                {
                                    if (i < BurglarsNr)
                                    ...
                                    plCorrectOpened(
                                        (LSObject)dbGetObject(ActionData(action, struct ActionOpen *)->ItemId), 0);
                                }
                        ...
                        case ACTION_CLOSE:
                            if (ActionStarted(plSys))
                            {
                                ...
                                {
                                    if (direction)
                                        lsSetObjectState(ActionData(action, struct ActionClose *)->ItemId,
                                                         Const_tcIN_PROGRESS_BIT, 1);
                                    else
                                        lsSetObjectState(ActionData(action, struct ActionClose *)->ItemId,
                                                         Const_tcIN_PROGRESS_BIT, 0);
                            ...
                            if (ActionEnded(plSys))
                            {
                                if (direction)
                                {
                                    ...
                                    plCorrectOpened(
                                        (LSObject)dbGetObject(ActionData(action, struct ActionClose *)->ItemId), 0);
                                }
                                else
                                {
                                    if (i < BurglarsNr)
                                    ...
                                    plCorrectOpened(
                                        (LSObject)dbGetObject(ActionData(action, struct ActionClose *)->ItemId), 1);
                                }
                ...
                if (!direction)
                    action = PrevAction(plSys);  // 2014-06-28 LucyG : ??? Compiler warning "action is assigned a value
                                                 // that is never used"
            }
            ...
            if (animate & PLANING_ANIMATE_NO)
            {
                ...
                if (CurrentTimer(plSys) != 0)
                {
                    for (a = (struct Action *)LIST_HEAD(h->Actions); NODE_SUCC(a); a = (struct Action *)NODE_SUCC(a))
                    {
                        ...
                        if (a == (struct Action *)h->CurrentAction) break;
                    }
                }
    ...
    if ((lastAreaId) && (lastAreaId != lsGetActivAreaID()))
    {
        ...
        lsInitActivArea(lastAreaId, livGetXPos(Planing_Name[CurrentPerson]), livGetYPos(Planing_Name[CurrentPerson]),
