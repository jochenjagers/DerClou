...
int dbCompare(KEY key1, KEY key2) { return (key1 == key2); }
...
struct dbObject *dbFindRealObject(uint32_t realNr, uint32_t offset, uint32_t size)
{
    ...
    for (objHashValue = 0; objHashValue < OBJ_HASH_SIZE; objHashValue++)
    {
             ...
             obj = (struct dbObject *)NODE_SUCC(obj))
        {
            if (obj->nr > offset)
            {
                if (size && (obj->nr > offset + size)) continue;

                if (obj->realNr == realNr) return obj;
            }
        }
    }
...
void dbSetLoadObjectsMode(ubyte mode) { ObjectLoadMode = mode; }
    ...
    if (mode == 0)  // Read mode
    {
        ...
        switch (objHd->type)
        {
            ...
            case Object_Building:
                ...
                if (bProfidisk) func(file, &((Building)obj)->DiskId, sizeof(unsigned char));
                break;
            case Object_LSArea:
            ...
            case Object_Environment:
                ...
                if (bProfidisk) func(file, &((Environment)obj)->PostzugDone, sizeof(unsigned char));
...
uint32_t dbGetObjectCountOfDB(uint32_t offset, uint32_t size)
{
    ...
    for (i = 0; i < OBJ_HASH_SIZE; i++)
    {
        ...
        {
            if ((obj->nr > offset) && (obj->nr < (offset + size))) count++;
        }
    }
...
void *dbGetObject(uint32_t nr)
{
    ...
    for (obj = (struct dbObject *)LIST_HEAD(objHash[objHashValue]); NODE_SUCC(obj);
         obj = (struct dbObject *)NODE_SUCC(obj))
    {
        if (obj->nr == nr) return dbGetObjectKey(obj);
    }

    return NULL;
}

uint32_t dbGetObjectNr(void *key) { return dbGetObjectReal(key)->nr; }
...
void *dbIsObject(uint32_t nr, uint32_t type)
{
         ...
         obj = (struct dbObject *)NODE_SUCC(obj))
    {
        if (obj->nr == nr)
        {
            if (obj->type == type) return dbGetObjectKey(obj);
...
struct ObjectNode *dbAddObjectNode(LIST *objectList, uint32_t nr, uint32_t flags)
{
    ...
    if (flags & OLF_INSERT_STAR) strcpy(name, "*");
...
void dbSortPartOfList(LIST *l, struct ObjectNode *start, struct ObjectNode *end,
...
int32_t dbSortObjectList(LIST **objectList, word (*processNode)(struct ObjectNode *, struct ObjectNode *))
{
    ...
    if (!LIST_EMPTY(*objectList))
    {
             ...
             n1 = (struct ObjectNode *)NODE_SUCC(n1), i++)
        {
            ...
            {
                for (n2 = (struct ObjectNode *)LIST_HEAD(newList); !pred && NODE_SUCC(n2);
                     n2 = (struct ObjectNode *)NODE_SUCC(n2))
                {
                    if (processNode(n1, n2) >= 0) pred = n2;
                }
            }
...
void dbInit(void)
{
    ...
    for (objHashValue = 0; objHashValue < OBJ_HASH_SIZE; objHashValue++)
    {
        if (!(objHash[objHashValue] = (LIST *)CreateList(0L))) NewErrorMsg(No_Mem, __FILE__, __func__, 4);
    }
...
void dbDone(void)
{
    ...
    for (objHashValue = 0; objHashValue < OBJ_HASH_SIZE; objHashValue++)
    {
        if (objHash[objHashValue]) RemoveList(objHash[objHashValue]);
    }

    if (ObjectList) RemoveList(ObjectList);
}
