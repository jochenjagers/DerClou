...
extern int32_t tcGuardDetectsGuy(LIST *roomsList, uword us_XPos, uword us_YPos, ubyte uch_ViewDirection,
                                 char *puch_GuardName, char *puch_LivingName);
...
extern uint32_t tcGuyUsesToolInPlayer(uint32_t persId, Building b, uint32_t toolId, uint32_t itemId,
                                      uint32_t neededTime);

extern void tcInsertGuard(LIST *list, LIST *roomsList, uword x, uword y, uword width, uword height, uint32_t guardId,
...
#define tcWeightPersCanCarry(p) \
    (((int32_t)(p)->Stamina + (int32_t)(p)->Strength) * 200) /* 0 - 100000 (100 kg) in gramm ! */
#define tcVolumePersCanCarry(p) \
    (((int32_t)(p)->Stamina + (int32_t)(p)->Strength) * 200) /* in cm3 -> max 1 m3 für eine Person */
...
#define tcCalcCarState(car) \
    (car->State = ((int32_t)(car->MotorState) + (int32_t)(car->BodyWorkState) + (int32_t)(car->TyreState)) / 3)
...
#define tcSetCarMotorState(car, v)                                                        \
    {                                                                                     \
        car->MotorState = (ubyte)ChangeAbs((int32_t)car->MotorState, (int32_t)v, 0, 255); \
        tcCalcCarState(car);                                                              \
    }
#define tcSetCarBodyState(car, v)                                                               \
    {                                                                                           \
        car->BodyWorkState = (ubyte)ChangeAbs((int32_t)car->BodyWorkState, (int32_t)v, 0, 255); \
        tcCalcCarState(car);                                                                    \
    }
#define tcSetCarTyreState(car, v)                                                       \
    {                                                                                   \
        car->TyreState = (ubyte)ChangeAbs((int32_t)car->TyreState, (int32_t)v, 0, 255); \
        tcCalcCarState(car);                                                            \
    }
...
#define tcAddBuildExactlyness(bui, v) \
    (bui->Exactlyness = (ubyte)ChangeAbs((int32_t)bui->Exactlyness, (int32_t)v, 0, 255))
