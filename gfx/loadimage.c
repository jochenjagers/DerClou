...
static SDL_Surface *gfxLoadILBM(ubyte *pFileBuffer, long ulSizeOfFile, const char *puch_Pathname)
{
	...
	ILBMHeader hdr;
	hdr.width = PeekW_BE(&pFileBuffer[currPos]);		currPos += 2;
	hdr.height = PeekW_BE(&pFileBuffer[currPos]);		currPos += 2;
	hdr.xOrigin = PeekW_BE(&pFileBuffer[currPos]);		currPos += 2;
	hdr.yOrigin = PeekW_BE(&pFileBuffer[currPos]);		currPos += 2;
	hdr.numPlanes = pFileBuffer[currPos];				currPos++;
	hdr.mask = pFileBuffer[currPos];					currPos++;
	hdr.compression = pFileBuffer[currPos];				currPos++;
	currPos++;	// padding
	hdr.transClr = PeekW_BE(&pFileBuffer[currPos]);		currPos += 2;
	hdr.xAspect = pFileBuffer[currPos];					currPos++;
	hdr.yAspect = pFileBuffer[currPos];					currPos++;
	hdr.pageWidth = PeekW_BE(&pFileBuffer[currPos]);	currPos += 2;
	hdr.pageHeight = PeekW_BE(&pFileBuffer[currPos]);	currPos += 2;
	...
	chunkSize = PeekL_BE(&pFileBuffer[currPos]);	currPos += 4;
	...
	if (currPos) {
		...
		if (numColors > 0) {
			...
			for (c = 0; c < numColors; c++) {
				...
				pc->r = pFileBuffer[currPos];	currPos++;
				pc->g = pFileBuffer[currPos];	currPos++;
				pc->b = pFileBuffer[currPos];	currPos++;
				pc->unused = 0;
			}
		}
	}
...
SDL_Surface *gfxLoadImage(const char *puch_Pathname)
{
	...
	if (!memcmp(&p_FileBuffer[0], idFORM, 4) &&
		!memcmp(&p_FileBuffer[8], idILBM, 4)) {
		// IFF ILBM (standard)
