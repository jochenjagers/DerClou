...
void tcDoneMeetBriggs(void)
{
    ...
    if (bProfidisk) AddTaxiLocation(68); /* baker street */
}
...
int32_t tcIsDeadlock(void)
{
    ...
    if (LIST_EMPTY(ObjectList))
    {
        ...
        total = comp->Bild + comp->Gold + comp->Geld + comp->Juwelen + comp->Statue + comp->Kuriositaet +
                comp->HistKunst + comp->GebrauchsArt + comp->Vase + comp->Delikates;
        ...
        for (n = (NODE *)LIST_HEAD(ObjectList); NODE_SUCC(n); n = (NODE *)NODE_SUCC(n))
        {
            ...
            if (tcGetCarPrice(car) < money) enough = 1;
        }
...
void tcDone1stBurglary(void)
{
    ...
    if (bProfidisk) AddTaxiLocation(75); /* train */
    ...
    if (bProfidisk) hasSet(Person_Marc_Smith, Car_Ford_Model_T__1926);
...
void tcDone2ndBurglary(void)
{
    ...
    if (bProfidisk) AddTaxiLocation(72); /* abbey */
...
void tcDone3rdBurglary(void)
{
    ...
    if (bProfidisk) AddTaxiLocation(74); /* downing */
...
void tcDone4thBurglary(void)
{
    ...
    if (bProfidisk) AddTaxiLocation(70); /* madame */
    ...
    if (bProfidisk) hasSet(Person_Marc_Smith, Car_Fiat_634_N_1943);
...
void tcDone5thBurglary(void)
{
    ...
    if (bProfidisk) AddTaxiLocation(77); /* tate */
...
void tcDone6thBurglary(void)
{
    ...
    if (bProfidisk) AddTaxiLocation(79); /* buckingham */
...
void tcDone7thBurglary(void)
{
    ...
    if (bProfidisk) AddTaxiLocation(81); /* bulstrode  */

    /* Jaguar kommt hier, da: 1. man braucht ihn für Villa, */
...
void tcDone9thBurglary(void)
{
    ...
    if (Env->MattIsInLove) AddTaxiLocation(61); /* lisson */
...
static void tcDoneFirstTimeLonelyInSouth(void)
{
    ...
    while (!ende)
    {
        ...
        if (((GetDay * 1440 + GetMinute) > (startTime + 420)) &&
            (!(Env->SouthhamptonHappened & 1))) /* nach 7 Stunden */
        {
        ...
        if (((GetDay * 1440 + GetMinute) > (startTime + 1020)) &&
            (!(Env->SouthhamptonHappened & 2))) /* nach 17 Stunden */
        {
        ...
        if (((GetDay * 1440 + GetMinute) > (startTime + 1500)) &&
            (!(Env->SouthhamptonHappened & 4))) /* nach 25 Stunden */
        {
        ...
        else
        {
            switch (activ)
            {
                ...
                case 3:
                    StopAnim();
                    Search.KaserneOk = 0;
                    if (!(burglary = plPlayer(Building_Starford_Kaserne, 0, NULL))) tcMattGoesTo(66); /* refresh! */
                    break;
                default:
                    break;
            }
...
int32_t tcIsLastBurglaryOk(void)
{
    ...
    if ((left->ul_Status & (1 << Const_tcON_OFF))) madeIt = 0;

    if (!(right->ul_Status & (1 << Const_tcON_OFF))) madeIt = 0;

    if (!has(Person_Matt_Stuvysunt, Loot_Dokument)) madeIt = 0;

    /* alle anderen müssen beim Auto sein... */
    for (i = 1; i < 4; i++)
        if ((Search.GuyXPos[i] < carXPos0) || (Search.GuyXPos[i] > carXPos1) || (Search.GuyYPos[i] < carYPos0) ||
