...
void Look(uint32_t locNr)
{
    ...
    while (choice != 2)
    {
        ...
        {
            case 0:
                bubble = txtGoKey(HOUSEDESC_TXT, NODE_NAME(GetNthNode(film->loc_names, (locNr))));
...
uint32_t tcTelefon(void)
{
    ...
    if (!(LIST_EMPTY(ObjectList)))
    {
        ...
        if (choice != GET_OUT)
        {
            ...
            choice =
                GET_OUT;  // 2014-06-28 LucyG : ??? Compiler warning "choice is assigned a value that is never used"
    }
    else
...
void tcWait(void)
{
    ...
    while ((!ende) && (minutes < 960))
    {
        ...
        if ((action & INP_LBUTTONP) || (action & INP_RBUTTONP) || (action & INP_ESC)) ende = 1;
    }
