...
uint32_t tcStartEvidence(void)
{
    ...
    for (n = (struct ObjectNode *)LIST_HEAD(guys), i = 0; NODE_SUCC(n); n = (struct ObjectNode *)NODE_SUCC(n), i++)
    {
        ...
        if (Search.SpotTouchCount[i]) div = 1;

        MyEvidence[i][0] = (((tcGetTrail(p[i], 0) * (int32_t)Search.WalkTime[i] * (max(1, 255 - guarded))) /
                             ((int32_t)Search.TimeOfBurglary + 1))) /
                           div;
        MyEvidence[i][1] = (((tcGetTrail(p[i], 1) * (int32_t)Search.WaitTime[i] * (max(1, 255 - guarded))) /
                             ((int32_t)Search.TimeOfBurglary + 1))) /
                           div;
        MyEvidence[i][2] = (((tcGetTrail(p[i], 2) * (int32_t)Search.WorkTime[i] * (max(1, 255 - guarded))) /
                             ((int32_t)Search.TimeOfBurglary + 1))) /
                           div;
        ...
        if (has(Person_Matt_Stuvysunt, Tool_Maske)) MyEvidence[i][6] = (MyEvidence[i][6] * 2) / 3;
    ...
    while (guyReady != ((1 << guyCount) - 1))
    {
        ...
        while (((1 << evidenceNr) & shownEvidence[guyNr]) && (j++ < 7)) evidenceNr = (evidenceNr + 1) % 7;
        ...
        if (j < 8) /* 8 stimmt! -> ober wird anschließend noch erhöht */
        {
            ...
            if (MyEvidence[guyNr][evidenceNr] > 0)
            {
                ...
                if (shownEvidence[guyNr] & 1) evidence->WalkTrail = MyEvidence[guyNr][0];

                if (shownEvidence[guyNr] & 2) evidence->WaitTrail = MyEvidence[guyNr][1];

                if (shownEvidence[guyNr] & 4) evidence->WorkTrail = MyEvidence[guyNr][2];

                if (shownEvidence[guyNr] & 8) evidence->KillTrail = MyEvidence[guyNr][3];

                if (shownEvidence[guyNr] & 16) evidence->CallTrail = MyEvidence[guyNr][4];

                if (shownEvidence[guyNr] & 32) evidence->PaperTrail = MyEvidence[guyNr][5];

                if (shownEvidence[guyNr] & 64) evidence->FotoTrail = MyEvidence[guyNr][6];
        ...
        if (shownEvidence[guyNr] == ((1 << 7) - 1)) guyReady |= (1 << guyNr);
    }
    ...
    for (i = 0; i < guyCount; i++)
    {
        ...
        if (p[i] != 0)
        {
            if (totalEvidence[i] > 255) totalEvidence[i] = 255;

            p[i]->KnownToPolice = (ubyte)(totalEvidence[i]);

            if (p[i]->KnownToPolice > tcPERSON_IS_ARRESTED) caught |= tcPersonWanted(OL_NR(GetNthNode(guys, i)));

            if (!caught)
                if (p[i] == dbGetObject(Person_Robert_Bull)) caught |= tcATraitor(Person_Robert_Bull);

            if (!caught)
                if (p[i] == dbGetObject(Person_Marc_Smith)) caught |= tcATraitor(Person_Marc_Smith);

            if (bProfidisk)
            {
                if (!caught)
                    if (p[i] == dbGetObject(Person_Phil_Ciggy)) caught |= tcATraitor(Person_Phil_Ciggy);
            }
        }
    }
    ...
    if (!(tcCarFound((Car)dbGetObject(Organisation.CarID), Search.TimeOfBurglary - Search.TimeOfAlarm)))
    {
        ...
        if (newStrike < (car->Strike + 40)) newStrike = ChangeAbs((int32_t)car->Strike, 40, 0, 255);
...
int32_t tcEscapeFromBuilding(uint32_t escBits)
{
    ...
    if (!(escBits & FAHN_SURROUNDED))
    {
        if ((escBits & FAHN_ALARM) || (escBits & FAHN_QUIET_ALARM))
        {
            ...
            if (timeLeft > 0) escapeSucc = FAHN_ESCAPED;
        }
        else
...
int32_t tcEscapeByCar(uint32_t escBits, int32_t timeLeft)
{
    ...
    if (escapeSucc == FAHN_ESCAPED)
    {
        if ((Search.BuildingId != Building_Tower_of_London) && (Search.BuildingId != Building_Starford_Kaserne))
    ...
    {
        if (escBits & FAHN_ALARM_ALARM) Say(BUSINESS_TXT, 0, gludo->PictID, "HINWEIS_6"); /* Alarmanlage */

        if (escBits & FAHN_ALARM_POWER) Say(BUSINESS_TXT, 0, gludo->PictID, "HINWEIS_8");

        if (escBits & FAHN_ALARM_TIMER) Say(BUSINESS_TXT, 0, gludo->PictID, "HINWEIS_6"); /* Alarmanlage */

        if (escBits & FAHN_ALARM_MICRO) Say(BUSINESS_TXT, 0, gludo->PictID, "HINWEIS_9");

        if (escBits & FAHN_ALARM_TIMECLOCK) Say(BUSINESS_TXT, 0, gludo->PictID, "HINWEIS_7"); /* Alarmanlage */

        if (escBits & FAHN_ALARM_GUARD) Say(BUSINESS_TXT, 0, gludo->PictID, "HINWEIS_10");
    }
    else
    {
        if (escBits & FAHN_QUIET_ALARM)
        {
            if (escBits & FAHN_ALARM_RADIO) Say(BUSINESS_TXT, 0, gludo->PictID, "HINWEIS_5");

            if (escBits & FAHN_ALARM_PATRO) Say(BUSINESS_TXT, 0, gludo->PictID, "HINWEIS_4");

            if (escBits & FAHN_ALARM_LOUDN) Say(BUSINESS_TXT, 0, gludo->PictID, "HINWEIS_3");
        }
    }

    if (escapeSucc != FAHN_ESCAPED) StopAnim();
...
int32_t tcCalcCarEscape(int32_t timeLeft)
{
    ...
    ubyte paint = 0;
    ubyte colortable[GFX_COLORTABLE_SIZE];
    char line[TXT_KEY_LENGTH];
    int32_t kmh = 0;
    int32_t ps = 0;
    int32_t i = 0;
    int32_t j = 0;
    int32_t YardsInFront = 0;
    int32_t length = 0;
    int32_t wayType = 0;
    int32_t unrealSpeed = 0;
    int32_t x = 0;
    int32_t xOldMatt = -1;
    int32_t xOldPoli = -1;
    ...
    if ((Organisation.BuildingID != Building_Tower_of_London) && (Organisation.BuildingID != Building_Starford_Kaserne))
    {
        ...
        do
        {
            ...
            if (Search.TimeOfAlarm) j += policeSpeed[wayType];
