...
uint32_t tcGoInsideOfHouse(uint32_t buildingID)
{
    ...
    if (count > 2)
    {
        for (i = count; i < 5; i++) RemoveNode(menu, NODE_NAME(GetNthNode(menu, count)));
        ...
        if (i) areaID = OL_NR(GetNthNode(areas, i - 1));
    }
...
void tcInsideOfHouse(uint32_t buildingID, uint32_t areaID, ubyte perc)
{
    ...
    node = lsGetSuccObject(LIST_HEAD(objects));
...
void tcShowObjectData(uint32_t areaID, NODE *node, ubyte perc)
{
    ...
    if (!LIST_EMPTY(ObjectList))
    {
        ...
        for (n = LIST_HEAD(ObjectList); NODE_SUCC(n); n = (NODE *)NODE_SUCC(n))
        {
            ...
            SetP(OL_DATA(n), hasLootRelationID, OL_DATA(n), GetP(OL_DATA(node), hasLootRelationID, OL_DATA(n)));
