...
void tcSellTool()
{
    ...
    if (LIST_EMPTY(tools)) Say(BUSINESS_TXT, 0, MATT_PICTID, "MATT_HAS_NO_TOOL");
...
void tcToolsShop(void)
{
    ...
    if (!(knows(Person_Matt_Stuvysunt, Person_Mary_Bolton))) knowsSet(Person_Matt_Stuvysunt, Person_Mary_Bolton);
