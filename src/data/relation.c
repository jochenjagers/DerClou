...
RELATION SetP(KEY leftKey, RELATION id, KEY rightKey, PARAMETER parameter)
{
    ...
    if (rd && CompareKey)
    {
        ...
        while (r)
        {
            ...
            {
                if (parameter != NO_PARAMETER) r->r_parameter = parameter;
                return id;
            }
...
PARAMETER GetP(KEY leftKey, RELATION id, KEY rightKey)
{
    ...
    if (rd && CompareKey)
    {
        ...
        while (r)
        {
            if (CompareKey(r->r_leftKey, leftKey) && CompareKey(r->r_rightKey, rightKey)) return r->r_parameter;
