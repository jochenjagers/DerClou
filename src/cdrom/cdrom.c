    ...
    if (SDL_InitSubSystem(SDL_INIT_CDROM) == 0)  // 2015-01-10 LucyG : changed from SDL_Init to SDL_InitSubSystem
    {
        if (SDL_CDNumDrives() > 0)
        {
            ...
            else
            {
                ...
                if (CDROM_Struct)
                {
                    if (SDL_CDStatus(CDROM_Struct) != CD_ERROR)
                    {  // 2014-07-12 LucyG : use SDL_CDStatus instead of CDROM_Struct->status
                        CDRomInstalled = 1;
                        return 1;
                    }
...
{
    if (CDROM_Struct) return SDL_CDStatus(CDROM_Struct);
...
int CDROM_SetGameCDDrive(void)
{
    ...
    if (num > 0)
    {
        for (i = 0; i < num; i++)
        {
            ...
            {
                if ((cdrom->numtracks == 24) && (cdrom->track[0].type == SDL_DATA_TRACK) &&
