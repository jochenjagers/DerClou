...
void plBuildHandler(NODE *n)
{
    ...
    if (OL_TYPE(n) == Object_Police) flags |= SHF_AUTOREVERS;

    InitHandler(plSys, OL_NR(n), flags);
}

void plClearHandler(NODE *n) { ClearHandler(plSys, OL_NR(n)); }

void plCloseHandler(NODE *n) { CloseHandler(plSys, OL_NR(n)); }
...
void plPrepareData(void)
{
    ...
    for (i = 0; i < PLANING_NR_PERSONS; i++) Planing_Weight[i] = 0;

    for (i = 0; i < PLANING_NR_PERSONS; i++) Planing_Volume[i] = 0;

    for (i = 0; i < PLANING_NR_GUARDS; i++) Planing_Guard[i] = 0;
}
...
{
    if (prepareMode & PLANING_GFX_LANDSCAPE) lsInitLandScape(objId, landscapMode);
...
void plUnprepareGfx(void)
{
    ...
    for (i = BurglarsNr; i < PersonsNr; i++) RemoveList(Planing_GuardRoomList[i - BurglarsNr]);
...
void plPrepareRel(void)
{
    ...
    if (!AddRelation(take_RelId)) NewErrorMsg(No_Mem, __FILE__, __func__, 0);
}
...
void plPrepareSys(uint32_t currPer, uint32_t objId, ubyte sysMode)
{
    ...
    if (objId) Planing_BldId = objId;

    if (sysMode & PLANING_INIT_PERSONSLIST)
    {
        if (PersonsList) RemoveList(PersonsList);

        if (BurglarsList) RemoveList(BurglarsList);
    ...
    if (sysMode & PLANING_HANDLER_SET) SetActivHandler(plSys, OL_NR(GetNthNode(PersonsList, CurrentPerson)));
}
