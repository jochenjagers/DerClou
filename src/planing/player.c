...
enum
{
    PLANING_PLAYER_PERSON_CHANGE = 0,
    PLANING_PLAYER_RADIO_ALL = 1,
    PLANING_PLAYER_RADIO_ONE = 2,
    PLANING_PLAYER_ESCAPE = 3
};

// Player defines
enum
{
    PLANING_EXHAUST_MAX = 240,
    PLANING_MOOD_MIN = 40,
    PLANING_LOUDNESS_STD = 5,
    PLANING_LOUDNESS_RADIO = 25,
    PLANING_LOUDNESS_OPEN_CLOSE = 14
};
...
static void UnableToWork(uint32_t current, uint32_t action)
{
    ...
    if (action != ACTION_WAIT) IncCurrentTimer(plSys, 1, 0);
}
...
static ubyte plGetMood(uint32_t time)
{
    ...
    for (i = 0; i < PLANING_NR_PERSONS; i++) guyId[i] = 0L;

    for (i = 0; i < BurglarsNr; i++) guyId[i] = OL_NR(GetNthNode(PersonsList, i));
...
static ubyte plCarTooFull(void)
{
    ...
    if (complete->TotalVolume > car->Capacity) ret = 1;
...
static void plPlayerAction(void)
{
    ...
    if (!(PD.timer % 15))
    {
        if (tcAlarmByLoudness(PD.bldObj, tcGetTotalLoudness(PD.currLoudness[0], PD.currLoudness[1], PD.currLoudness[2],
                                                            PD.currLoudness[3])))
            Search.EscapeBits |= FAHN_QUIET_ALARM | FAHN_ALARM_LOUDN;
    }
#endif

    if (PD.bldId == Building_Tower_of_London) patroCounter = 9;

    if (PD.bldId == Building_Starford_Kaserne) patroCounter = 30;
    ...
    {
        if (!(--PD.alarmTimer)) Search.EscapeBits |= FAHN_QUIET_ALARM | FAHN_ALARM_TIMER;
    }
#endif
    ...
    if ((Search.EscapeBits & FAHN_QUIET_ALARM) || (Search.EscapeBits & FAHN_ALARM))
    {
        ...
        if (PD.realTime >= (Search.TimeOfAlarm + PD.bldObj->PoliceTime))
        {
            ...
            for (i = 0; i < PLANING_NR_PERSONS; i++) PD.handlerEnded[i] = 1;
    ...
    for (i = 0; i < PersonsNr; i++)
    {
        if ((i >= BurglarsNr) || !PD.handlerEnded[i])
        {
            ...
            else if ((PD.action = NextAction(plSys)))
            {
                if (!(PD.timer % 12) && (i >= BurglarsNr) && !(Search.EscapeBits & FAHN_ALARM_GUARD))
                {
                    ...
                    for (j = 0; j < BurglarsNr; j++)
                    {
                        if (tcGuardDetectsGuy(Planing_GuardRoomList[i - BurglarsNr], xpos, ypos, dir, Planing_Name[i],
                                              Planing_Name[j]))
                        {
                            if ((PD.bldId == Building_Starford_Kaserne) && j) break;
                            ...
                            PD.guardKO[((Police)dbGetObject(OL_NR(GetNthNode(PersonsList, i))))->LivingID -
                                       BurglarsNr] = 3;
                            break;
                        }
                    }
                ...
                if (!(PD.timer % 3) && (i < BurglarsNr) && (livWhereIs(Planing_Name[i]) == lsGetActivAreaID()))
                {
                    if (PD.bldId != Building_Starford_Kaserne)
                    {
                        if (!(Search.EscapeBits & FAHN_ALARM_MICRO) &&
                            tcAlarmByMicro(livGetXPos(Planing_Name[i]), livGetYPos(Planing_Name[i]),
                                           PD.currLoudness[i]))
                        {
                            Search.DeriTime += PLANING_DERI_ALARM;
                ...
                switch (PD.action->Type)
                {
                        ...
                        if (i < BurglarsNr)
                        {
                            ...
                            else
                            {
                                ...
                                if (livCanWalk(Planing_Name[i]))
                                {
                                    ...
                                    Search.Exhaust[i] =
                                        tcGuyInAction(OL_NR(GetNthNode(PersonsList, i)), Search.Exhaust[i]);
                                    Search.WalkTime[i]++;

                                    if (i == CurrentPerson)
                                    {
                                        lsInitScrollLandScape(ActionData(PD.action, struct ActionGo *)->Direction,
                                                              LS_SCROLL_PREPARE);
                                        DoScrolling = 1;
                                    }
                                }
                                ...
                                {
                                    if (livWhereIs(Planing_Name[i]) == lsGetActivAreaID()) UnableToWork(i, ACTION_GO);
                                }
                            }
                        }
                        else
                    ...
                    case ACTION_WAIT:
                        if (i < BurglarsNr) UnableToWork(i, ACTION_WAIT);
                        break;
                        ...
                        if (i < BurglarsNr)
                        {
                            ...
                            if (ActionStarted(plSys))
                            {
                                ...
                                InitSignal(plSys, OL_NR(GetNthNode(PersonsList, i)),
                                           ActionData(PD.action, struct ActionSignal *)->ReceiverId);

#ifndef PLAN_IS_PERFECT
                                if (tcAlarmByRadio(PD.bldObj)) Search.EscapeBits |= FAHN_QUIET_ALARM | FAHN_ALARM_RADIO;
#endif
                            }
                            ...
                            {
                                struct plSignal *sig =
                                    IsSignal(plSys, OL_NR(GetNthNode(PersonsList, i)),
                                             ActionData(PD.action, struct ActionSignal *)->ReceiverId);

                                if (sig) CloseSignal(sig);
                            }
                        }
                        break;
                    ...
                    case ACTION_WAIT_SIGNAL:
                        ...
                        {
                            struct plSignal *sig =
                                IsSignal(plSys, ActionData(PD.action, struct ActionWaitSignal *)->SenderId,
                    ...
                    case ACTION_USE:
                        if (i < BurglarsNr)
                        {
                            ...
                            if (ActionStarted(plSys))
                            {
                                if (plIsStair(ActionData(PD.action, struct ActionUse *)->ItemId))
                                    livLivesInArea(Planing_Name[i],
                                                   StairConnectsGet(ActionData(PD.action, struct ActionUse *)->ItemId,
                                ...
                                else
                                {
                                    ...
                                    if (dbIsObject(ActionData(PD.action, struct ActionUse *)->ItemId, Object_Police))
                                    {
                                        ...
                                        {
                                            PD.currLoudness[i] = tcGetToolLoudness(
                                                OL_NR(GetNthNode(PersonsList, i)),
                                                ActionData(PD.action, struct ActionUse *)->ToolId, Item_Wache);
                                            PD.guardKO[((Police)dbGetObject(
                                                            ActionData(PD.action, struct ActionUse *)->ItemId))
                                                           ->LivingID -
                                                       BurglarsNr] = 1;
                                            PD.changeCount += 5;
                                            ...
                                            realTime =
                                                tcGuyUsesToolInPlayer(OL_NR(GetNthNode(PersonsList, i)), PD.bldObj,
                                                                      ActionData(PD.action, struct ActionUse *)->ToolId,
                                                                      Item_Wache, needTime / PLANING_CORRECT_TIME);
                                    ...
                                    {
                                        if (!CHECK_STATE(
                                                lsGetObjectState(ActionData(PD.action, struct ActionUse *)->ItemId),
...
#ifndef PLAN_IS_PERFECT
                                        if (tcGetDanger(OL_NR(GetNthNode(PersonsList, i)),
                                                        ActionData(PD.action, struct ActionUse *)->ToolId,
                                                        ((LSObject)dbGetObject(
                                                             ActionData(PD.action, struct ActionUse *)->ItemId))
                                                            ->Type))
                                        ...
                                        realTime = tcGuyUsesToolInPlayer(
                                            OL_NR(GetNthNode(PersonsList, i)), PD.bldObj,
                                            ActionData(PD.action, struct ActionUse *)->ToolId,
                                            ((LSObject)dbGetObject(ActionData(PD.action, struct ActionUse *)->ItemId))
                                                ->Type,
                                    ...
                                    {
                                        Search.DeriTime +=
                                            ((realTime * PLANING_CORRECT_TIME - needTime) * 5) / PLANING_CORRECT_TIME;

                                        IncCurrentTimer(plSys, realTime * PLANING_CORRECT_TIME - needTime, 0);
                                    }

                                    plPersonLearns(OL_NR(GetNthNode(PersonsList, i)),
                                                   ActionData(PD.action, struct ActionUse *)->ToolId);
                                }
                            }
                            ...
                            if (!(Search.EscapeBits & FAHN_ALARM_ALARM) &&
                                tcAlarmByTouch(ActionData(PD.action, struct ActionUse *)->ItemId))
                            {
                                Search.DeriTime += PLANING_DERI_ALARM;
                                ...
                                if (!plIgnoreLock(ActionData(PD.action, struct ActionUse *)->ItemId))
                                {
                                    if (plIsStair(ActionData(PD.action, struct ActionUse *)->ItemId))
                                    {
                                        if (i == CurrentPerson)
                                        {
                                            ...
                                            lsInitActivArea(newAreaId, livGetXPos(Planing_Name[i]),
                                                            livGetYPos(Planing_Name[i]), Planing_Name[i]);
                                    ...
                                    else if (dbIsObject(ActionData(PD.action, struct ActionUse *)->ItemId,
                                                        Object_Police))
                                    {
                                    ...
                                    else
                                    {
                                                ...
                                                Const_tcLOCK_UNLOCK_BIT))
                                        {
                                            ...
                                            lsSetObjectState(ActionData(PD.action, struct ActionUse *)->ItemId,
                                                             Const_tcLOCK_UNLOCK_BIT, 1);

                                            if (((Tool)dbGetObject(ActionData(PD.action, struct ActionUse *)->ToolId))
                                                    ->Effect &
                                                Const_tcTOOL_OPENS)
                                            {
                                                lsSetObjectState(ActionData(PD.action, struct ActionUse *)->ItemId,
                                                                 Const_tcOPEN_CLOSE_BIT, 1);
                                                plCorrectOpened((LSObject)dbGetObject(
                                                                    ActionData(PD.action, struct ActionUse *)->ItemId),
                                                                1);
                                            }
                                        ...
                                        else
                                        {
                                                     ...
                                                     ->Type == Item_Fenster))
                                            {
                                                ...
                                                lsWalkThroughWindow(
                                                    (LSObject)dbGetObject(
                                                        ActionData(PD.action, struct ActionUse *)->ItemId),
                                                    livGetXPos(Planing_Name[i]), livGetYPos(Planing_Name[i]), &xpos,
                                                    &ypos);
                                ...
                                else
                                {
                                    ...
                                    if (CHECK_STATE(state, Const_tcON_OFF))
                                    {
                                        ...
                                        if (plIgnoreLock(ActionData(PD.action, struct ActionUse *)->ItemId) ==
                                            PLANING_POWER)
                                        {
                                            lsSetSpotStatus(ActionData(PD.action, struct ActionUse *)->ItemId,
                                                            LS_SPOT_ON);
                    ...
                    case ACTION_TAKE:
                        if (i < BurglarsNr)
                        {
                            ...
                            if (ActionStarted(plSys)) PD.changeCount++;

                            if (Ask(dbGetObject(ActionData(PD.action, struct ActionTake *)->ItemId), hasLoot(i),
                                    dbGetObject(ActionData(PD.action, struct ActionTake *)->LootId)) &&
                                plObjectInReach(i, ActionData(PD.action, struct ActionTake *)->ItemId))
                            {
                                lsSetObjectState(ActionData(PD.action, struct ActionTake *)->ItemId,
                                ...
                                if (ActionEnded(plSys))
                                {
                                    uint32_t newValue = GetP(
                                        dbGetObject(ActionData(PD.action, struct ActionTake *)->ItemId), hasLoot(i),
                                        dbGetObject(ActionData(PD.action, struct ActionTake *)->LootId));

                                    Planing_Weight[i] +=
                                        ((Loot)dbGetObject(ActionData(PD.action, struct ActionTake *)->LootId))->Weight;
                                    Planing_Volume[i] +=
                                        ((Loot)dbGetObject(ActionData(PD.action, struct ActionTake *)->LootId))->Volume;

                                    lsSetObjectState(ActionData(PD.action, struct ActionTake *)->ItemId,
                                                     Const_tcIN_PROGRESS_BIT, 0);

                                    if ((ActionData(PD.action, struct ActionTake *)->ItemId >= 9701) &&
                                        (ActionData(PD.action, struct ActionTake *)->ItemId <= 9708))
                                    {
                                    ...
                                    {
                                        if (CHECK_STATE(
                                                lsGetObjectState(ActionData(PD.action, struct ActionTake *)->ItemId),
                                                Const_tcTAKE_BIT))
                                        {
                                            lsSetObjectState(ActionData(PD.action, struct ActionTake *)->ItemId,
                                                             Const_tcACCESS_BIT, 0);
                                            lsTurnObject((LSObject)dbGetObject(
                                                             ActionData(PD.action, struct ActionTake *)->ItemId),
                                                         LS_OBJECT_INVISIBLE, LS_NO_COLLISION);
                                        }
                                    }

                                    UnSet(dbGetObject(ActionData(PD.action, struct ActionTake *)->ItemId), hasLoot(i),
                                          dbGetObject(ActionData(PD.action, struct ActionTake *)->LootId));

                                    if (Ask(dbGetObject(OL_NR(GetNthNode(PersonsList, i))), take_RelId,
                                            dbGetObject(ActionData(PD.action, struct ActionTake *)->LootId)))
                                    {
                                        uint32_t oldValue =
                                            GetP(dbGetObject(OL_NR(GetNthNode(PersonsList, i))), take_RelId,
                                                 dbGetObject(ActionData(PD.action, struct ActionTake *)->LootId));

                                        SetP(dbGetObject(OL_NR(GetNthNode(PersonsList, i))), take_RelId,
                                             dbGetObject(ActionData(PD.action, struct ActionTake *)->LootId),
                                             oldValue + newValue);
                                    }
                                    else
                                        SetP(dbGetObject(OL_NR(GetNthNode(PersonsList, i))), take_RelId,
                                             dbGetObject(ActionData(PD.action, struct ActionTake *)->LootId), newValue);
                    ...
                    case ACTION_DROP:
                        if (i < BurglarsNr)
                        {
                            ...
                            if (Ask(dbGetObject(OL_NR(GetNthNode(PersonsList, i))), take_RelId,
                                    dbGetObject(ActionData(PD.action, struct ActionDrop *)->LootId)))
                            {
                                lsSetObjectState(ActionData(PD.action, struct ActionDrop *)->ItemId,
                                                 Const_tcIN_PROGRESS_BIT, 1);
                                ...
                                if (ActionEnded(plSys))
                                {
                                    ...
                                    Planing_Weight[i] -=
                                        ((Loot)dbGetObject(ActionData(PD.action, struct ActionDrop *)->LootId))->Weight;
                                    Planing_Volume[i] -=
                                        ((Loot)dbGetObject(ActionData(PD.action, struct ActionDrop *)->LootId))->Volume;

                                    lsSetObjectState(ActionData(PD.action, struct ActionDrop *)->ItemId,
                                                     Const_tcIN_PROGRESS_BIT, 1);

                                    if ((ActionData(PD.action, struct ActionDrop *)->ItemId >= 9701) &&
                                        (ActionData(PD.action, struct ActionDrop *)->ItemId <= 9708))
                                    {
                                        lsAddLootBag(livGetXPos(Planing_Name[i]), livGetYPos(Planing_Name[i]),
                                                     ActionData(PD.action, struct ActionDrop *)->ItemId - 9700);
                                    ...
                                    else
                                    {
                                        if (CHECK_STATE(
                                                ...
                                                Const_tcTAKE_BIT))
                                            lsTurnObject((LSObject)dbGetObject(
                                                             ActionData(PD.action, struct ActionDrop *)->ItemId),
                                                         LS_OBJECT_VISIBLE, LS_COLLISION);
                                    }

                                    SetP(dbGetObject(ActionData(PD.action, struct ActionDrop *)->ItemId), hasLoot(i),
                                         dbGetObject(ActionData(PD.action, struct ActionDrop *)->LootId), newValue);
                                    UnSet(dbGetObject(OL_NR(GetNthNode(PersonsList, i))), take_RelId,
                                          dbGetObject(ActionData(PD.action, struct ActionDrop *)->LootId));
                    ...
                    case ACTION_OPEN:
                        if (ActionStarted(plSys))
                        {
                            ...
                            {
#ifndef PLAN_IS_PERFECT
                                if (CHECK_STATE(lsGetObjectState(ActionData(PD.action, struct ActionOpen *)->ItemId),
                        ...
                        if ((i >= BurglarsNr) || plIgnoreLock(ActionData(PD.action, struct ActionOpen *)->ItemId) ||
                            CHECK_STATE(lsGetObjectState(ActionData(PD.action, struct ActionOpen *)->ItemId),
                                        Const_tcLOCK_UNLOCK_BIT))
                        {
                            if ((i >= BurglarsNr) || plIgnoreLock(ActionData(PD.action, struct ActionOpen *)->ItemId) ||
                                !CHECK_STATE(lsGetObjectState(ActionData(PD.action, struct ActionOpen *)->ItemId),
                                             Const_tcOPEN_CLOSE_BIT))
                            {
                                if (i < BurglarsNr)
                                {
                                    ...
                                    Search.WorkTime[i]++;
                                    lsSetObjectState(ActionData(PD.action, struct ActionOpen *)->ItemId,
                                                     Const_tcIN_PROGRESS_BIT, 1);
                                ...
                                if (ActionEnded(plSys))
                                {
                                    ...
                                    plCorrectOpened(
                                        (LSObject)dbGetObject(ActionData(PD.action, struct ActionOpen *)->ItemId), 1);
                    ...
                    case ACTION_CLOSE:
                        ...
                        if ((i >= BurglarsNr) || plIgnoreLock(ActionData(PD.action, struct ActionClose *)->ItemId) ||
                            CHECK_STATE(lsGetObjectState(ActionData(PD.action, struct ActionOpen *)->ItemId),
                                        Const_tcLOCK_UNLOCK_BIT))
                        {
                            if ((i >= BurglarsNr) ||
                                plIgnoreLock(ActionData(PD.action, struct ActionClose *)->ItemId) ||
                                CHECK_STATE(lsGetObjectState(ActionData(PD.action, struct ActionClose *)->ItemId),
                                            Const_tcOPEN_CLOSE_BIT))
                            {
                                if (i < BurglarsNr)
                                {
                                    PD.currLoudness[i] = PLANING_LOUDNESS_OPEN_CLOSE;
                                    Search.WorkTime[i]++;
                                    lsSetObjectState(ActionData(PD.action, struct ActionClose *)->ItemId,
                                                     Const_tcIN_PROGRESS_BIT, 1);
                                ...
                                if (ActionEnded(plSys))
                                {
                                    ...
                                    plCorrectOpened(
                                        (LSObject)dbGetObject(ActionData(PD.action, struct ActionClose *)->ItemId), 0);
                    ...
                    case ACTION_CONTROL:
                        ...
                        if (tcGuardChecksObject(
                                (LSObject)dbGetObject(ActionData(PD.action, struct ActionControl *)->ItemId)))
                        {
                            if (!(Search.EscapeBits & FAHN_ALARM_GUARD))
                            {
                        ...
                        if (((LSObject)dbGetObject(ActionData(PD.action, struct ActionUse *)->ItemId))->Type ==
                            Item_Stechuhr)
        ...
        else
        {
            ...
            if (PD.handlerEnded[i] == 32) livAnimate(Planing_Name[i], ANM_DUSEL, 0, 0);
        }
    }

    if (DoScrolling) lsScrollLandScape((ubyte)-1);
    ...
    for (i = 0; i < PLANING_NR_PERSONS; i++)
    {
        if (!PD.handlerEnded[i]) PD.ende = 0;
    }

    if (PD.ende) SetMenuTimeOutFunc(NULL);
}
...
int32_t plPlayer(uint32_t objId, uint32_t actionTime, ubyte (*actionFunc)(uint32_t, uint32_t))
{
    ...
    if ((activ = plOpen(objId, PLANING_OPEN_READ_BURGLARY, &fh)) == PLANING_OPEN_OK)
    {
        if (!(l = LoadSystem(fh, plSys)))
        {
            if (!(l = plLoadTools(fh)))
            {
                ...
                for (i = 0; i < PLANING_NR_GUARDS; i++) PD.guardKO[i] = 0;
                    ...
                    switch (activ)
                    {
                        ...
                        case PLANING_PLAYER_RADIO_ONE:
                            if (BurglarsNr > 2)
                            {
                                ...
                                choice1 = CurrentPerson ? OL_NR(GetNthNode(BurglarsList, 0))
                ...
                inpSetWaitTicks(0); /* normales Ausmassß */
                ...
                for (i = 0; i < PersonsNr; i++) livTurn(Planing_Name[i], LIV_DISABLED); /* alle Sprites ausschalten! */
                ...
                for (i = 0; i < BurglarsNr; i++)
                {
                         ...
                         n = (struct ObjectNode *)NODE_SUCC(n))
                    {
                        if (has(Person_Matt_Stuvysunt, OL_NR(n)))
                        {
                            ...
                            uint32_t newValue =
                                GetP(dbGetObject(OL_NR(GetNthNode(PersonsList, i))), take_RelId, dbGetObject(OL_NR(n)));
    ...
    {
        if (activ == PLANING_OPEN_ERR_NO_PLAN) plMessage("NO_PLAN", PLANING_MSG_REFRESH | PLANING_MSG_WAIT);
    }

    if (Search.BuildingId == Building_Starford_Kaserne) Search.KaserneOk = tcIsLastBurglaryOk();
    ...
    if (ret) return tcEscapeByCar(Search.EscapeBits, timeLeft);
