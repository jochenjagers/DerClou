...
void lsInitLandScape(uint32_t bID, ubyte mode) /* initialisiert das Landschaftsmodul */
{
    ...
    // lootbags müssen vor den Livings initialisiert werden, da sie
    // einer kleiner Prioritaet haben (unter den Maxis erscheinen)
    for (i = 9701; i <= 9708; i++)
    {
        LSObject lso = (LSObject)dbGetObject(i);

        // OffsetFact wird am PC nicht für den Offset in der Plane
...
void lsLoadGlobalData(uint32_t bld, uint32_t ul_AreaId)
{
    ...
    if (!(LoadRelations(fileName, 0))) NewErrorMsg(Disk_Defect, __FILE__, __func__, 2);
}
...
void lsInitObjectDB(uint32_t bld, uint32_t areaID)
{
    ...
    if (dbLoadAllObjects(fileName, 0))
    {
        ...
        if (!(LoadRelations(fileName, 0))) NewErrorMsg(Disk_Defect, __FILE__, __func__, 3);
    }
    else
...
void lsDoneLandScape(void)
{
    ...
    if (ls)
    {
        ...
        if (ls->p_DoorRefreshList) RemoveList(ls->p_DoorRefreshList);
...
void lsDoneActivArea(uint32_t newAreaID) { livSetAllInvisible(); /* MOD 14-01-94 */ }
