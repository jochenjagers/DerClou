...
ubyte lsScrollLandScape(ubyte direction)
{
    ...
    ubyte collis = 0;
    ubyte back = 0;
    ubyte speed = ls->uch_ScrollSpeed;

    if (direction != (ubyte)-1) collis = lsInitScrollLandScape(direction, LS_SCROLL_PREPARE);
...
ubyte lsInitScrollLandScape(ubyte direction, ubyte mode)
{
    ...
    int32_t tx = 0;
    int32_t ty = 0;
    int32_t speed = (int32_t)l->uch_ScrollSpeed;
    ubyte collis = 0;
...
ubyte lsIsCollision(uword x, uword y, ubyte direction)
{
    ...
    for (i = 0; i < 14; i++) color[i] = 255;

    // Kollision überprüfen
    // Da sich das Maxi immer um 2 Pixel weiterbewegt, müssen Pixel
    // hintereinander abgefragt werden, außerdem wird immer an jeder
    ...
    for (i = 0; i < 14; i++)
    {
        if ((color[i] != LS_COLLIS_COLOR_0) && (color[i] != LS_COLLIS_COLOR_1) && (color[i] != LS_COLLIS_COLOR_2) &&
    ...
    // wenn die aeusseren kollidieren, die inneren aber nicht -> keine Kollision (Tür!)
    if ((color[0] || color[1]) && (color[6] || color[5]) && !color[3]) collis = 0;

    if ((color[7] || color[8]) && (color[13] || color[12]) && !color[10]) collis = 0;

    // Raender überprüfen!
    if (x <= 2) collis |= LS_COLLIS_LEFT;
    if (x >= (LS_MAX_AREA_WIDTH - LS_PERSON_X_SIZE - 3)) collis |= LS_COLLIS_RIGHT;

    if (y <= 2) collis |= LS_COLLIS_UP;
    if (y >= (LS_MAX_AREA_HEIGHT - LS_PERSON_Y_SIZE - 3)) collis |= LS_COLLIS_DOWN;

    if (GamePlayMode & GP_COLLISION_CHECKING_OFF) collis = 0;
