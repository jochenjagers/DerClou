...
static void lsSafeRectFill(word x, word y, word x1, word y1)
{
    ...
    if ((x1 > x) && (y1 > y)) gfxNCH4RectFill(l_wrp, x, y, x1, y1);
}
...
void lsSetVisibleWindow(uword x, uword y)
{
    ...
    if ((wX = x - halfX) < 0) wX = 0;

    if ((wY = y - halfY) < 0) wY = 0;
...
static void lsShowRooms(void)
{
    if (GamePlayMode & GP_SHOW_ROOMS)
    {
        ...
        for (room = LIST_HEAD(rooms); NODE_SUCC(room); room = NODE_SUCC(room))
        {
            ...
            gfxRectFill(l_wrp, myroom->us_LeftEdge, myroom->us_TopEdge, myroom->us_LeftEdge + myroom->us_Width - 1,
...
void lsBuildScrollWindow(void)
{
    ...
    for (i = 0; i < LS_FLOORS_PER_COLUMN; i++)
    {
        ...
        {
            // falls kein Boden vorhanden ist, mit Kollisionsfarbe füllen!
    ...
    for (node = (NODE *)LIST_HEAD(ls->p_ObjectRetrieval); NODE_SUCC(node); node = (NODE *)NODE_SUCC(node))
    {
        ...
        // Statue müssen wegen der unsauberen Scheiße, speziell refresht werden
        // würg!!!
        if (lso->Type == Item_Statue)
            if (lso->uch_Visible != LS_OBJECT_VISIBLE) lsRefreshStatue(lso);
    }
    ...
    /* auf gehts Burschn, schpü ma wos flottes */
...
void lsTurnObject(LSObject lso, ubyte status, ubyte Collis)
{
    ...
    if (status == LS_OBJECT_VISIBLE) LS_SET_OBJECT(ls->p_CurrFloor[floorIndex].uch_FloorType);

    if (lso->Type == Item_Mikrophon) LS_SET_MICRO_ON_FLOOR(ls->p_CurrFloor[floorIndex].uch_FloorType);
}
...
void lsSetObjectState(uint32_t objID, ubyte bitNr, ubyte value)
{
    ...
    // bei einer Stechuhr darf sich der Status nicht ändern!
    if (object->Type != Item_Stechuhr)
    {
        if (value == 0) object->ul_Status &= (0xffffffff - (1L << bitNr));

        if (value == 1) object->ul_Status |= (1L << bitNr);
    }
}
                    ...
                    if (areaId[i] == s->ul_AreaId)
                    {
                        if (((struct Spot *)s)->p_CurrPos)
                        {
                            ...
                            if ((us_XPos[i] < x + size - 2) && (us_XPos[i] > x + 2))
                                if ((us_YPos[i] < y + size - 2) && (us_YPos[i] > y + 2)) Search.SpotTouchCount[i]++;
                        }
                    }
                }
            }
...
void lsPatchObjects(void)
{
    ...
    for (n = (NODE *)LIST_HEAD(ls->p_ObjectRetrieval); NODE_SUCC(n); n = (NODE *)NODE_SUCC(n))
    {
        ...
        lso->uch_Size = item->Size;  // Grö▀en stimmen nicht überall überein!
        ...
        if (OL_NR(n) == tcLAST_BURGLARY_LEFT_CTRL_OBJ) lso->ul_Status |= (1 << Const_tcON_OFF);
...
void lsCalcExactSize(LSObject lso, uword *x0, uword *y0, uword *x1, uword *y1)
{
    ...
    // keine Ahnung warum bei Bild & Gemälde, OPEN_CLOSE_BIT & HORIZ_VERT_BIT
...
static void lsInitDoorRefresh(uint32_t ObjId)
{
    ...
    for (drn = (struct LSDoorRefreshNode *)LIST_HEAD(ls->p_DoorRefreshList); NODE_SUCC(drn);
         drn = (struct LSDoorRefreshNode *)NODE_SUCC(drn))
        if (drn->lso == lso) found = 1;
    ...
    if (!found)
    {
        ...
        gfxNCH4PutNCH4ToMCGA(l_wrp->p_BitMap, LS_PREPARE_BUFFER, lso->us_DestX, lso->us_DestY, destX, destY, width,
                             height, 160, 320);

        lsPrepareToXMSRastPort(&LS_DOOR_REFRESH_XMS_RP);

        drn = (struct LSDoorRefreshNode *)CreateNode(ls->p_DoorRefreshList, sizeof(struct LSDoorRefreshNode), 0L);
...
// restauriert aus einem XMS Buffer den Hintergrund einer Tür
{
    ...
    for (drn = (struct LSDoorRefreshNode *)LIST_HEAD(ls->p_DoorRefreshList); NODE_SUCC(drn);
         drn = (struct LSDoorRefreshNode *)NODE_SUCC(drn))
        if (drn->lso == lso) break;

    width = lso->uch_Size;
    height = lso->uch_Size;

    gfxNCH4OLMCGAToNCH4Mask(LS_PREPARE_BUFFER, l_wrp->p_BitMap, drn->us_XOffset, drn->us_YOffset, lso->us_DestX,
                            lso->us_DestY, width, height, 320, 160);
}
