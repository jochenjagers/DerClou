...
static const int index_car[] = {0x03, 0x04, 0x05, 0x0B, 0x0C, 0x0D, 0x13, 0x14, 0x15};

static const int index_mod[] = {0x00, 0x01, 0x02, 0x08, 0x09, 0x0A, 0x10, 0x11, 0x12};

static const int freq_table[] = {0x016B, 0x0181, 0x0198, 0x01B0, 0x01CA, 0x01E5,
...
static void hsc_set_volume(HSC_File *pHSC, int channel, int volume_car, int volume_mod)
{
    ...
    if (volume_car != 0xFF && volume_mod != 0xFF && (pHSC->instrument[pHSC->hsc_ch_inst[channel]].feedback_fm & 1))
    {
        volume_mod |= pHSC->instrument[pHSC->hsc_ch_inst[channel]].mod_ksl_volume & 0xC0;
...
static void hsc_process_row(HSC_File *pHSC)
{
    ...
    if (pHSC->hsc_effect03)
    {
        ...
        else
        {
            for (i = 0; i < 9; i++)
            {
                hsc_set_volume(pHSC, i, 0x3F - pHSC->hsc_effect03_line * 2 + 1, 0x3F - pHSC->hsc_effect03_line * 2 + 1);
            }
        }
    }
    ...
    for (channel = 0; channel < 9; channel++)
    {
            ...
            default:
            {
                ...
                if (pHSC->instrument[pHSC->hsc_ch_inst[channel]].slide >> 0x04)
                {
                    pHSC->hsc_ch_note[channel] +=
                        (int)((pHSC->instrument[pHSC->hsc_ch_inst[channel]].slide >> 0x04) * 1.31);
                }
        ...
        if (effect != 0x00)
        {
            ...
            {
                hsc_set_volume(pHSC, channel, (effect - 0xC0) * 0x4, (effect - 0xC0) * 0x4);
            }
            /* Set speed */
...
static void MXR_DestroyInputHSC(MXR_InputHSC *pInput) { MXR_MemFree(pInput, sizeof(MXR_InputHSC)); }
