...
void tcTheAlmighty(uint32_t time) { tcMovePersons(2, time); }
...
void StdInit(void)
{
    ...
    ShowTime(0); /* Zeit sollte nach der Anim gezeigt werden, sonst Probleme mit
                    Diskversion */
    ...
    if (!(GamePlayMode & GP_MUSIC_OFF))
    {
        switch (GetCurrentScene()->EventNr)
        {
            ...
            default:
            {
                ...
                if (strcmp(sndGetCurrSoundName(), "street1.bk") && strcmp(sndGetCurrSoundName(), "street2.bk") &&
...
void StdDone(void)
{
    ...
    while (!SceneArgs.ReturnValue)
    {
        ...
        if (bProfidisk)
        {
            if (GetCurrentScene()->EventNr == SCENE_PROFI_26)
            {
                ...
                if (env->PostzugDone) SceneArgs.Moeglichkeiten &= ~INVESTIGATE;
            }
        }
    ...
    if (!(SceneArgs.Ueberschrieben)) gfxChangeColors(l_wrp, 5L, GFX_FADE_OUT, 0L);
...
void tcGetLastName(char *Name, char *dest, uint32_t maxLength)
{
    ...
    if (strlen(lastName) > maxLength) lastName[maxLength] = '\0';
...
void tcCutName(char *Name, char Sign, uint32_t maxLength)
{
    ...
    if ((j = strlen(Source)) > maxLength) j = maxLength;

    for (i = j - 1; i >= 0; i--)
        if (Source[i] == Sign) Source[i] = '\0';
