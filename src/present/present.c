...
void DrawPresent(LIST *present, ubyte firstLine, struct RastPort *rp, ubyte max)
{
    ...
    for (i = firstLine, j = 4; (max < firstLine + NRBLINES) ? i < max : i < firstLine + NRBLINES; i++, j += 9)
    {
        ...
        {  // 2014-07-13 LucyG : forgot this in v0.4
...
ubyte Present(uint32_t nr, const char *presentationText, void (*initPresentation)(uint32_t, LIST *, LIST *))
{
    ...
    while (!exitLoop)
    {
        ...
        if (action & INP_MOUSE)
        {
            ...
            while ((y < 9) && (firstVis > 0))
            { /* Scroll up */
                DrawPresent(presentationData, --firstVis, u_wrp, maxNr);
                y = inpGetMouseY(u_wrp);
            }

            while ((y > 50) && (y <= 59) && (firstVis < (maxNr - 5)))
            { /* Scroll down */
                DrawPresent(presentationData, ++firstVis, u_wrp, maxNr);
                y = inpGetMouseY(u_wrp);
            }
...
void prSetBarPrefs(struct RastPort *p_RP, uword us_BarWidth, uword us_BarHeight, ubyte uch_FCol, ubyte uch_BCol,
                   ubyte uch_TCol)
{
