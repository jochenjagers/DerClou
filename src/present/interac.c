...
ubyte GetExtBubbleActionInfo(void) { return ExtBubbleActionInfo; }

void SetMenuTimeOutFunc(void *func) { MenuTimeOutFunc = (void (*)())func; }

ubyte ChoiceOk(ubyte choice, ubyte xit, LIST *l)
{
    if (choice == GET_OUT) return 0;

    if (choice == xit) return 0;

    if (l && !LIST_EMPTY(l))
    {
        struct ObjectNode *objNode = (struct ObjectNode *)GetNthNode(l, choice);

        if (!objNode->nr && !objNode->type && !objNode->data) return 0;
    }
...
void DrawMenu(LIST *menu, ubyte nr, int32_t mode)
{
    ...
    {
        if (m2) gfxPrintExact(m_wrp, m2, x + 8 * (nr - 1) - lastx, TXT_2ND_MENU_LINE_Y);
    }
}
...
char SearchActiv(word delta, ubyte activ, uint32_t possibility, ubyte ub_max)
{
    do
    {
        ...
        if (activ > ub_max) activ = ub_max;

        if (possibility & (1L << activ)) return (char)activ;
...
char SearchMouseActiv(uint32_t possibility, ubyte ub_max) /* MOD : 14.12.93 hg */
{
    ...
    if ((y > TXT_1ST_MENU_LINE_Y - 10) && (y < TXT_2ND_MENU_LINE_Y + 10))
    {
        ...
        if (y > TXT_1ST_MENU_LINE_Y + 7) activ++;
...
void RefreshMenu(void)
{
    ...
    if (refreshMenu)
    {
        ...
        for (i = 0; i < ub_max; i++)
        {
            if (refreshPoss & (1L << i)) DrawMenu(refreshMenu, i, INACTIV_POSS);
        }
...
ubyte Menu(LIST *menu, uint32_t possibility, ubyte activ, void (*func)(ubyte), uint32_t waitTime)
{
    ...
    if (menu && !LIST_EMPTY(menu))
    {
        ...
        for (i = 0; i < ub_max; i++)
        {
            if (possibility & (1L << i)) DrawMenu(menu, i, INACTIV_POSS);
        }
        ...
        if (waitTime) inpSetWaitTicks(waitTime);

        while (!ende)
        {
            action = INP_LEFT | INP_RIGHT | INP_UP | INP_DOWN | INP_LBUTTONP;

            if (waitTime) action |= INP_TIME;
            ...
            if ((action & INP_ESC) || (action & INP_RBUTTONP)) return GET_OUT;

            if (action & INP_LBUTTONP) ende = TRUE;
...
ubyte Bubble(LIST *bubble, ubyte activ, void (*func)(ubyte), uint32_t waitTime)
{
    ...
    if (activ == GET_OUT) activ = 0;

    if (activ >= NRBLINES) firstVis = activ - NRBLINES + 1;
    ...
    else
    {
        ...
        if (CurrentBubbleType == THINK_BUBBLE) gfxShow((uword)10, GFX_NO_REFRESH | GFX_OVERLAY, 0, -1, -1);
    }
    ...
    if (waitTime)
    {
        ...
        if (action & INP_LBUTTONP) activ = 1;

        if ((action & INP_ESC) || (action & INP_RBUTTONP) || (action & INP_TIME)) activ = GET_OUT;
                    ...
                    if ((x >= X_OFFSET) && (x <= X_OFFSET + INT_BUBBLE_WIDTH))
                    {
                        ...
                        else if ((y > 48) && (y <= 52) && (firstVis < (l_max - 5))) /* Scroll down */
                        {
                            while ((y > 48) && (y <= 52) && (firstVis < (l_max - 5)))
                            {
                                ...
                                if (activ > (l_max - 1)) activ = l_max - 1;
                        ...
                        else if ((y >= 4) && (y <= 48))
                        {
                            ...
                            if (newactiv != activ)
                            {
                                ...
                                if (activ < firstVis) activ = firstVis;

                                if (activ > firstVis + 4) activ = firstVis + 4;
                ...
                else if (action & INP_MOUSEWHEEL)
                {
                    if (action & INP_UP)
                    {
                        if (activ > 0)
                        {
                            ...
                            while ((activ > 0) && cl)
                            {
                                ...
                                if (*NODE_NAME(GetNthNode(bubble, activ)) == '*') break;
                            }

                            if (activ < firstVis) firstVis = activ;
                    ...
                    else if (action & INP_DOWN)
                    {
                        if (activ < l_max - 1)
                        {
                            ...
                            if (activ > (firstVis + NRBLINES - 1)) firstVis = activ - NRBLINES + 1;
    ...
    if (activ != GET_OUT)
    {
        if (*NODE_NAME(GetNthNode(bubble, 0L)) != '*') activ = 0;
    }
