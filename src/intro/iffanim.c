...
typedef struct
{
    ...
    int nch;       // number of channels: 0 no audio, 1 mono, 2 stereo (left, right interleaved), other values aren't
                   // supported
    ...
    int *dataoffset;  // list of audio sample start, which starts playing at current frame (for every frame), in bytes,
...
struct IffAnim
{
    ...
    int compressed;  // indicates compression type of first frame (0 or 1, only byterun is supported) (other frames have
                     // delta compression)
    unsigned char dcompressions[256];  // bit array indicates which delta compression methods are used in the anim
                                       // (there may be mixed compression modes)
    ...
    int disp_bpp;    // display format (doesn't change), in common supported format: (1..8 bit converted to 8, ham to 24
                     // bpp)
    int disp_pitch;  // number of byte per scanline (size of dispframe in bytes: h * disp_pitch), can be considered as
                     // always 0
    ...
    char *prev_disp_frame;  // previous display frame (a player may need this for double buffering with resize handling
                            // without creating an extra frame copy)
    ...
    int framesize;    // size of a decoded frame in bytes (each scanline has a multiple of 16 bit -> number of bytes is
                      // even)
    ...
    int loop;  // if unset, there is an error when trying to load the next frame of the last frame, else the animation
               // restarts
...
static int IffAnim_ConvertHamTo24bpp(IffAnim *anim, void *dst_, void *src_, void *cmap_, int w, int h, int hambits,
                                     int dst_pitch);
static int IffAnim_BitplanarToChunky(IffAnim *anim, void *dst_, void *src_, int w, int h, int bitssrc, int bitsdst,
                                     int dst_pitch);
                                     ...
                                     int dst_pitch)
{
    ...
    int data = 0;
    int mode = 0;                                 // parts of a HAM value
    ...
    unsigned char *dst = NULL;  // working pointers
    ...
    {  // for every line
        ...
        {  // for every pixel of line
            ...
            {
                case 0x0:
                    memcpy(colbuf, cmap + (data * 3), 3);
                    break;  // RGB values from color map
                case 0x1:
                    colbuf[2] = data << datashift;
                    break;  // change blue upper bits
                case 0x2:
                    colbuf[0] = data << datashift;
                    break;  // change red upper bits
                case 0x3:
                    colbuf[1] = data << datashift;
                    break;  // change green upper bits
            }
                                     ...
                                     int dst_pitch)
{
    ...
    for (k = 0; k < h; k++)
    {
        ...
        for (j = 0; j < w; j++)
        {
            ...
            srcBitOfs = j;
            downshift = 7 - (srcBitOfs &
                             0x7);  // bit order in a bitplane byte: 0x1 masks bit 7, 0x80 masks bit 0 of color index
            for (i = 0; i < bitssrc; i++)
            {
...
static int IffAnim_DecodeByteRun(IffAnim *anim, void *dst_, void *data_, int datasize, int w, int h, int bpp, int mask)
{
    ...
    for (i = 0; i < h; i++)
    {
        ...
        while (j > 0)
        {
            ...
            {  // multiple times the same byte value
                n = -n + 1;
                val = src[possrc++];
                if (n > j)
                {  // overflow protection
...
static int IffAnim_DecodeByteVerticalDelta(IffAnim *anim, void *dst_, void *data_, int w, int bpp)
{
    ...
    int op = 0;
    int val = 0;  // holds opcode , data value
    ...
    for (k = 0; k < bpp; k++)
    {
        ...
        if (ofssrc)
        {  // no change in plane if pointer index is 0
...
static int IffAnim_DecodeLSVerticalDelta7(IffAnim *anim, void *dst_, void *data_, int w, int bpp, int int32_t_data)
{
    ...
    if (int32_t_data)
    {
        wordsize = 4;
        ncolumns = (w + 31) / 32;
        if (((w + 15) / 16 * 2) != ((w + 31) / 32 * 4))
        {  // relating to the width of a video frame, possibly for the last
           // column of each plane one must copy only 16 bit words,
            half = TRUE;
        }
    }
    else
    ...
    for (i = 0; i < bpp; i++)
    {
        ...
        if (p_op)
        {  // if opcode pointer index not 0 => plane is modified
            // for each column
            for (j = 0; j < ncolumns; j++)
            {
                ...
                while (opcnt)
                {
                    ...
                    if ((wordsize == 2) || (half && ((j + 1) == ncolumns)))
                    {  // 2 bytes per data word, or 16 bit of last column with 32 bit byte wordsize
                        // SAME_OP, opcode is 0
                        if (op == 0)
                        {
            ...
            for (j = 0; j < ncolumns; j++)
            {
                ...
                while (opcnt)
                {
                    ...
                    {  // 4 bytes per data word
                        ...
                        if (op == 0)
                        {
                            op = (data[p_op] << 24) | (data[p_op + 1] << 16) | (data[p_op + 2] << 8) |
                                 data[p_op + 3];  // number of same words
                            p_op += 4;
            ...
            case 1:
                ...
                for (g = 0; g < g_cnt; g++)
                {
                    ...
                    {  // number of vertical steps
                        for (d = 0; d < bpp; d++)
                        {  // loop thru planes, a delta byte for each plane
                            if (r_flag)
                            {
                                *i_ptr ^= *delta++;
                            }
...
static int fget16(FILE *file) { return ((fgetc(file) << 8) | fgetc(file)); }
    ...
    */
    ...
    for (i = 0; i < anim->nframes; i++)
    {
...
#ifdef THECLOU_DEBUG_ALLOC
                tabuf = (char **)MemAllocDbg(
                    sizeof(char *) * anim->audio.n, __FILE__,
                    "IffAnim_ReadFrames 2");  // temporary audio data buffer list, an allocated block for each frame
#else
                tabuf = (char **)MemAlloc(
                    sizeof(char *) *
                    anim->audio.n);  // temporary audio data buffer list, an allocated block for each frame
#endif
                if (!tabuf)
                {
                    return (FALSE);
...
#ifdef THECLOU_DEBUG_ALLOC
                tabufsize =
                    (int *)MemAllocDbg(sizeof(int) * anim->audio.n, __FILE__,
                                       "IffAnim_ReadFrames 3");  // list of size of a block in bytes (for each frame)
#else
                tabufsize =
                    (int *)MemAlloc(sizeof(int) * anim->audio.n);  // list of size of a block in bytes (for each frame)
#endif
                if (!tabufsize)
                {
                    return (FALSE);
        ...
        if ((anim->audio.nch > 0) && (anim->audio.n > i) && tabuf && tabufsize)
        {
            ...
            {
                if (IffAnim_read_SBDY(anim, file, ILBMsize, &(tabuf[i]), &(tabufsize[i])) < 0)
                {  // if error occurs
                    break;
                }
            }
        ...
        if ((anim->frame[i].delta_compression != 0) && (anim->frame[i].delta_compression != 5) &&
            (anim->frame[i].delta_compression != 7) && (anim->frame[i].delta_compression != 8) &&
        ...
        if (body && anim->compressed)
        {
...
#ifdef THECLOU_DEBUG_ALLOC
            framemem = (char *)MemAllocDbg(anim->framesize, __FILE__,
                                           "IffAnim_ReadFrames 5");  // memory for the RLE decompression is needed
#else
            ...
            IffAnim_DecodeByteRun(anim, framemem, anim->frame[i].data, anim->frame[i].datasize, anim->w, anim->h,
                                  anim->bpp, anim->mask);
            ...
            if ((anim->frame[i].delta_compression != 0) && (anim->frame[i].delta_compression != 1))
            {  // in some files the delta compression is not set correctly for the first BODY frame, only 0
               // (uncompressed) or 1 (XOR map) is allowed (which can be RLE compression although)
...
static int IffAnim_PrintInfo(IffAnim *anim)
{
    ...
    sprintf(anim->formatinfo,
            ...
            anim->nframes, anim->w, anim->h, anim->bpp, anim->disp_bpp, anim->mask, anim->ham ? "yes" : "no",
            anim->ehb ? "yes" : "no", anim->lentime);
    ...
    n = 0;
    for (i = 0; i < 256; i++)
    {  // list all delta compression modes
    ...
    if (IffAnim_GetAudioFormat(anim, NULL, NULL, NULL))
    {
        ...
        strcat(anim->formatinfo, buffer);
        if ((anim->audio.nch != 0) && (anim->audio.bps != 0))
...
static int IffAnim_DecodeFrame(IffAnim *anim, char *dstframe, int index)
{
    ...
    switch (anim->frame[index].delta_compression)
    {
        ...
        case 7:
            return (IffAnim_DecodeLSVerticalDelta7(anim, dstframe, anim->frame[index].data, anim->w, anim->bpp,
                                                   (anim->frame[index].bits & 0x1) ? TRUE : FALSE));
            break;
        // Short/Long vertical delta method 8
        case 8:
            return (IffAnim_DecodeLSVerticalDelta8(anim, dstframe, anim->frame[index].data, anim->w, anim->bpp,
                                                   (anim->frame[index].bits & 0x1) ? TRUE : FALSE));
            break;
        case 74:
...
void *IffAnim_GetCmap(IffAnim *anim) { return (anim->disp_cmap); }

void *IffAnim_GetPrevFrame(IffAnim *anim) { return (anim->prev_disp_frame); }

void *IffAnim_GetPrevCmap(IffAnim *anim) { return (anim->prev_disp_cmap); }

int IffAnim_Reset(IffAnim *anim)
{
    anim->frameno = -1;  // so next frame is 0
    if (anim->frame[0].delta_compression)
    {                                                 // set to black, if frame 0 has delta compression
...
int IffAnim_NextFrame(IffAnim *anim)
{
    ...
    {  // if last frame
        ...
        {  // handle looping
            ...
            {  // continue at frame 2 (skip the first 2)
                anim->frameno = 1;
            }
            else
            {
...
int IffAnim_ConvertFrame(IffAnim *anim)
{
    ...
    if (anim->ham)
    {
        IffAnim_ConvertHamTo24bpp(anim, anim->disp_frame, anim->curframe, anim->curcmap, anim->w, anim->h, anim->bpp,
                                  anim->disp_pitch);
    }
    else
    {
        IffAnim_BitplanarToChunky(anim, anim->disp_frame, anim->curframe, anim->w, anim->h, anim->bpp, anim->disp_bpp,
                                  anim->disp_pitch);
    }
...
int IffAnim_GetDelayTimeOriginal(IffAnim *anim) { return (anim->frame[anim->frameno].reltime); }
