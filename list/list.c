...
void FreeList(void *list) { MemFree(list, LIST_SIZE(list)); }

void *AddNode(void *list, void *node, void *predNode)
{
    if (!predNode) predNode = list;
...
void *AddTailNode(void *list, void *node) { return AddNode(list, node, LIST_TPRED(list)); }

void *AddHeadNode(void *list, void *node) { return AddNode(list, node, list); }
...
uint32_t GetNodeNr(void *list, char *name) { return GetNodeNrByAddr(list, GetNode(list, name)); }
...
{
    if (predNode) *predNode = NODE_PRED(node);

    return RemNode(node);
}

void *UnLink(void *list, char *name, void **predNode) { return UnLinkByAddr(list, GetNode(list, name), predNode); }
...
void ReplaceNode(void *list, char *name, void *newNode) { ReplaceNodeByAddr(list, GetNode(list, name), newNode); }
...
uint32_t ReadList(void *list, uword size, char *fileName, uword diskId)
{
    ...
    if (fh = dskOpen(fileName, "r", diskId))
    {
        ...
        {
            if (buffer[0] == ';') continue;  // overread comments

            if (!buffer[0]) continue;  // skip empty line
