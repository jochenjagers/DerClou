...
void tcSaveTheClou(void)
{
	...
	ubyte location[TXT_KEY_LENGTH];
	ubyte date[TXT_KEY_LENGTH];
	ubyte RootPath[TXT_KEY_LENGTH];
	char pathname[TXT_KEY_LENGTH];
	LIST  *games = CreateList (0);
	NODE  *game;
	uword activ;
...
{
	ubyte line[TXT_KEY_LENGTH], loaded = 0;
	ubyte RootPath[TXT_KEY_LENGTH];
...
{
	ubyte line[TXT_KEY_LENGTH], loaded;
	LIST *games = CreateList (0);
	LIST *origin = CreateList (0);
	ulong activ;
	Player player;
...
ubyte tcSaveChangesInScenes(ubyte *fileName)
{
	long i;
	ubyte back = 0;
	FILE *file;
...
ubyte tcLoadChangesInScenes(ubyte *fileName)
{
	long i;
	ubyte buffer[TXT_KEY_LENGTH],back = 1;
	ulong eventNr, count;
	FILE *file;
	struct Scene *sc;
