...
uint32_t tcOrganisation(void)
{
    ...
    /***********************************************
     * erstes oder gemerktes Gebäude aktivieren !
     */
    Organisation.BuildingID = 0;
    Organisation.CarID = 0;
    Organisation.DriverID = 0;
    ...
    if (!LIST_EMPTY(ObjectList)) Organisation.BuildingID = OL_NR(LIST_HEAD(ObjectList));

    if (Organisation.BuildingID)
    {
        if (!has(Person_Matt_Stuvysunt, Organisation.BuildingID)) tcOrganisationSetBuilding();
    }
    else
        tcOrganisationSetBuilding();

    // auf alle Fälle erinnern an aktuellen Stand
    ...
    {
        if (!has(Person_Matt_Stuvysunt, Organisation.CarID)) tcOrganisationSetCar();
    }
    else
    ...
    if (!LIST_EMPTY(ObjectList)) Organisation.DriverID = OL_NR(LIST_HEAD(ObjectList));

    if (!tcMakeCarOk()) return 0L;
    ...
    while (!ende)
    {
        ...
        switch (activ)
        {
            ...
            case 4:
                ...
                break;
            case 5:
                if (!Organisation.BuildingID) Organisation.BuildingID = tcChooseDestBuilding(Organisation.BuildingID);
    ...
    else
    {
        ...
        if (ChoiceOk(choice = Bubble(list, 0, 0L, 0L), GET_OUT, list))
        {
            ...
            else
            {
                ...
                if (!LIST_EMPTY(ObjectList)) rememberUnSet(Person_Matt_Stuvysunt, OL_NR(LIST_HEAD(ObjectList)));
